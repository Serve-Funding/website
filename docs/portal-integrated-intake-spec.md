...
> ## ⚠ STATUS: MOSTLY SCRATCHED OR HELD — this is a record, not a plan
>
> **Decided 2026-09-09** by Kyler after Tim's weekly dev meeting. Reasoning and Tim's own words:
> `Brain/knowledge/playbooks/build-priorities-2026-09-09.md`. Read that before building anything
> described below.
>
> | Piece | Status |
> |---|---|
> | One-page form (`/get-started`, `OnePageForm`, `triageCompleteAnswers`) | **SCRATCHED.** Not parked — cancelled. |
> | Program-fit screen + eligibility gate (`/api/program-fit`) | **HELD** on data, not quality. Website side stays in PR #84; portal side in Serve-Platform #1291. Both left open. |
> | `financing_type` question on `/discover` | **DEFERRED** — adds a visible question during the funnel freeze. Portal side landed in Serve-Platform #1292. |
> | `company_state` on the form | **DEFERRED**, same reason. Still wanted: state drives the licensing screen. |
> | Document-upload handoff (magic link) | **SHIPPING.** Portal side in #1292; website CTA on `/discover`'s choice point. |
> | `getRoleType` partner-default fix | **SHIPPING.** |
>
> **Why the one-page form is dead**, on this document's own evidence: 90 days of Umami to
> 2026-09-08 shows 128 form loads → 34 answered question one → 26 gave contact details → 22
> finished. The entire loss is screen one; length was never the measured problem, so a one-page
> layout addressed something the numbers do not show. Tim: *"don't do too much too fast. Like if you
> do a ton of changes, you don't actually know what made it better."* The already-shipped set (#81
> amount-first, #82 pop-up removed, #83 above-fold CTA, #85 non-blocking verification) now gets 1–2
> months to read before anything else on the form moves. At ~600 visitors per 90 days, two competing
> intake forms could never have resolved against each other anyway.
>
> **Why program-fit is held:** the gate cannot be built correctly on today's matrix.
> `restricted_industries` is populated on 16% of 313 products and `states_excluded` on 8%, and the
> form's 16 curated industries share no vocabulary with the lender restriction strings — so a
> cannabis dispensary selects "Other" and the gate's most important case cannot fire. That profile's
> raw count measured **23**, i.e. it would have been shown "more than 20 lenders". Unblocking it is
> a matrix backfill, not code.
>
> **Where the handoff actually earns its keep, per Tim:** send the portal upload link **before the
> first call on referred deals**, so documents are pre-loaded when Michael or Camden gets on the
> phone. That needs no new code — `app/actions/client-access.ts` already mints admin-initiated
> links. It is a process change. The website path is weaker by comparison, because the handoff mints
> only for an email the verifier returns `valid`, excluding `catchall` — common on real corporates.
>
> Everything below is preserved as written, including measurements and reasoning that still hold.
> Read statements of intent as historical.

**Date:** 2026-09-09. **Owner:** Kyler.
...
form intake (`src/app/api/verify-contact`).

**Superseded on 2026-09-09 — #85 made that verification NON-BLOCKING.** It still runs and still
flags on the lead notification, but it no longer turns a submission away, and undeliverable
addresses are tracked so the false-positive rate can be measured before anyone reinstates a hard
stop. The handoff guard is unaffected: `emailIsMintable` reads `verification.email.result`, which
#85 left alone — only the always-null `hardFail` field went away. So an unverifiable address now
means *no upload link*, not *no lead*, which is the better trade in both directions.
